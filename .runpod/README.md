...
**Configuration file:** You can also supply a full `vllm serve` config file instead of (or alongside) env vars. Mount a YAML anywhere in the container and set the `VLLM_CONFIG_FILE` env var to its path. Use the same key names as `vllm serve` — hyphens and underscores both work:
